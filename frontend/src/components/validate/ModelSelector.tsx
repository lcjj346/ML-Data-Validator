...
export default function ModelSelector({ selected, onSelect }: Props) {
  ...
  if (loading) return <p className="text-slate-400 text-sm">Loading models...</p>;
  if (!models.length) {
    return (
  ...
  return (
    <div className="relative">
      ...
      <select
        ...
        className="w-full bg-slate-800/80 border border-white/10 rounded-xl pl-9 pr-3 py-2.5 text-sm focus:border-cyan-500 focus:outline-none focus:ring-1 focus:ring-cyan-500/50 appearance-none cursor-pointer transition-all duration-200 hover:border-white/20"
      >
        {models.map((m) => (
